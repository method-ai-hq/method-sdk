...
export async function runSaved(saved:any, flags:ReturnType<typeof parse>['values'], client:MethodClient) {
  ...
  const request=prior??{run_id:existsSync(identity)?JSON.parse(readFileSync(identity,'utf8')).id:randomUUID(),workflow_id:saved.workflow_id,version_id:saved.version_id,server:client.server};
