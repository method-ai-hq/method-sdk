...
const edited = "JSON {file, workflow}. The workflow field contains the method and is kept for compatibility. Writes the local draft.";
...
export const commandHelp: Record<string, Command> = {
  ...
  authoring: { usage: "method authoring [start|concepts|execution|example|recipes|recovery|commands|all]", purpose: "Read the installed authoring guide. Available offline.", arguments: "Default topic: start, including the complete approved Method. Supporting files are installed beside the CLI; their path is printed. all prints the full manual.", result: "Markdown text on stdout. No changes.", errors: "Unknown topic: lists the valid topics; exit 1.", example: "method authoring all > method-guide.md" },
  ...
  "step add": { usage: "method step add FILE --id ID --value-file STEP.yaml", purpose: "Add a complete operation.", arguments: "Supply do or ask and the bindings and outputs needed by the step in STEP.yaml. purpose, reading, checks, and limit overrides are optional. Alternatively use --kind run --runtime PROFILE --entrypoint FILE. Agents can use --kind agent --instructions-file FILE; the default model is the local Codex agent.", result: edited, errors: commonErrors, example: "method step add task.method --id copy --value-file copy.yaml" },
  ...
  "check set": { usage: "method check set FILE STEP_ID (--json JSON|--value-file FILE)", purpose: "Set the operation's independent check.", arguments: "Current methods require an equals/count/present/file object, a run check, or an agent check. Put plain-English criteria in an agent check prompt. --text and --text-file apply only to older formats.", result: edited, errors: "Unknown step, invalid check, or conflicting value flags.", example: "method check set task.method search --value-file check.yaml" },
  ...
  validate: { usage: "method validate FILE [--config FILE] [--workspace DIR]", purpose: "Check the definition and local setup without running the work.", arguments: "Checks data, names, dependencies, templates, declared files, executables, environment variables, runtime profiles and tool bindings. Defaults to runtime.json beside the Method. --workspace selects the helper folder and default config folder. Relative paths in config resolve from the config folder.", result: "JSON reports definition, local_setup, executed:false, and valid. Missing setup or invalid definition exits 1 with an exact error. Legacy Methods get definition checks only.", errors: "The error identifies the invalid field or reference.", example: "method validate task.method" },
...
Use method doctor to check the installed Node and Codex tools.`, result: "Progress and final status text; local result.json and run evidence; dashboard run link when synced. The runtime executes the method's declared scripts, calls, agents, and checks. Executes trusted local processes; changes declarations do not enforce permissions. Current runs exit 0 on completion, 1 on failure, and 2 when human input is needed. Older saved runs can use exit 2 for a failed run.", errors: "Missing inputs/access, failed check, timeout, unsafe resume/version mismatch, upload failure. See recovery. Never retry a business write without inspecting its saved changes.", example: "method run wf_example --version version_example --config runtime.json --workspace . --inputs inputs.json", remote: true },
};

commandHelp.migrate = { usage: "method migrate FILE --model PROFILE --timeout-ms N --max-agent-turns N --max-model-requests N [--output FILE]", purpose: "Create a current Method version from an older method.", arguments: "Choose the model profile and finite limits explicitly. The source stays unchanged. Review migration warnings and prepare matching operator configuration before running.", result: "Converted method and warnings.", errors: "Missing model or limits; output file already exists.", example: "method migrate old.method --model worker --timeout-ms 60000 --max-agent-turns 8 --max-model-requests 12 --output current.method" };
...
export const guideTopics = ["start", "concepts", "execution", "example", "recipes", "recovery", "commands"] as const;
export function authoringGuide(topic = "start"): string {
  const topics: Record<string, () => string> = { start: () => start, execution: () => execution, concepts: () => concepts, example: () => example, recipes: () => recipes, recovery: () => recovery, commands: () => "# Command reference\n\nLocal authoring commands edit draft files. create, save and update publish method versions. run executes a saved version.\n\n" + serverHelp + "\n" + exitHelp + "\n\nCommon errors:\n" + commonErrors + "\n\n" + Object.keys(commandHelp).map(name => renderCommand(name, false)).join("\n") + "\n## Copy a message: syntax reference\n\n```yaml\n" + exampleWorkflow + "```\n\n## Example: build with editing commands\n\nThis small syntax example shows the optional editing commands. Use an empty folder.\n\n```sh\n" + exampleScript() + "\n```\n" };
  if (topic === "all") return guideTopics.filter(key => key !== "example").map(key => topics[key]!()).join("\n\n");
...
Execution and evidence: run, runs, logs, sync.
Account access: status, login, logout, devices, revoke.

Read the approved example, write the files, validate, run a sample, inspect the result, and save.
