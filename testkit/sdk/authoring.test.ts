...
it("edits stable named steps and preserves the file after invalid changes",async()=>{
 const {file}=setup();writeFileSync(file,JSON.stringify(method()));await localAuthoring(["step","update",file,"copy","--json",JSON.stringify({do:{kind:"agent",model:"default",prompt:"Return message unchanged.",tools:[]}})]);expect(loadWorkflow(readDocument(file)).steps.copy?.do).toMatchObject({prompt:"Return message unchanged."});
